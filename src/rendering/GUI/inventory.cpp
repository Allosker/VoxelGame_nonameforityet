	...
	font{ font_ }
{
	...
	m_moving_item.text.setScale(ItemStackGUI::g_scale_text_hover);
}
...
void Render::GUI::Inventory::update(const Window& window, const ItemTypeManager& itm, Hotbar& hotbar) noexcept
{
	...
	for (std::size_t i{}; i < m_slots.size(); i++)
		if (m_slots[i].contains(point))
		{
			...
			m_items_slots[i].text.setScale(ItemStackGUI::g_scale_text_hover);
			isWithinSlot = true;
		}
		else
		{
			m_items_slots[i].setScale(Hotbar::g_scale_item_coef_rest);
			m_items_slots[i].text.setScale(ItemStackGUI::g_scale_text_rest);
		}
	...
	if (!isWithinSlot)
		for (std::size_t i{}; i < hotbar.getSlots().size(); i++)
			if (hotbar.getSlots()[i].contains(point))
			{
				...
				hotbar.getItems()[i].text.setScale(ItemStackGUI::g_scale_text_hover);
			...
			else
			{
				...
				hotbar.getItems()[i].text.setScale(ItemStackGUI::g_scale_text_rest);
			}
