...
Render::GUI::Font::Font(const types::path& path, uint8 btmp_size) noexcept
	: m_btmp_size{ btmp_size }
{
	load_font(path);
}

Render::GUI::Font::~Font() noexcept
{
	glDeleteTextures(1, &m_texture_id);
}

void Render::GUI::Font::load_font(const types::path& path) noexcept
{
...
/*private*/ Render::Image Render::GUI::Font::create_bitmap(const types::path& path) noexcept
{
	FT_Library ft;
