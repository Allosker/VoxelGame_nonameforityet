...
Render::GUI::Hotbar::Hotbar(const Render::Texturing::Texture& texture_slot, const ItemTypeManager& itm, const Font* font_)
	: font{ font_ }
{
...
std::optional<GameWorld::Inventory::Item> Render::GUI::Hotbar::getSlot_at(std::size_t index) const noexcept
{
	...
	return std::make_optional(m_slots[index].stack_item);
}
...
{
	m_slots[m_cursor_position].stack_item = item;
}
...
{
	if (m_disabled)
		return m_slots[m_cursor_position].stack_item;
...
void Render::GUI::Hotbar::disable() noexcept
{
	...
	m_slots[m_cursor_position].setScale(1);
	...
	const ItemTypeManager& itm) noexcept
{
	for (std::size_t i{}; i < m_slots.size(); i++)
	{
		...
		if (m_slots[i].stack_item.id != 0)
		{
			m_items_slots[i].updateSprite(mapTextureUvs(m_slots[i].stack_item.id, itm));
...
/*private*/ void Render::GUI::Hotbar::create_new_slots(const Render::Texturing::Texture& texture_slot) noexcept
{
	...
	(ItemStack2D
	{
		g_slot_size, { g_slot_size.x / 2, g_slot_size.y / 2 }, types::Rect<types::uvs>{ {}, texture_slot.getSize() }, {}, *font
	});
