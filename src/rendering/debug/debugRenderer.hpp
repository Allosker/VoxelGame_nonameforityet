...
namespace render::debug
{
	...
	public:
		...
		struct VertexList
		{
			...
			std::vector<Data::VertexRGBA> vertices;
			...
			const auto renderList = [&](VertexList& list, bool depthEnabled, GLenum type)
				{
					...
					if (list.dirty)
					{
						glBufferData(GL_ARRAY_BUFFER, list.vertices.size() * sizeof(Data::VertexRGBA), list.vertices.data(), GL_DYNAMIC_DRAW);
						list.dirty = false;
					}
